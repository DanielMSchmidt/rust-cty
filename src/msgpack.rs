...
mod conformance {
    ...
    use crate::msgpack::{implied_type, marshal, unmarshal};
    use crate::{Type, Value, convert};
    ...
    //
    // Upstream TestRoundTrip is a single table covering several concepts at once.
    ...
    mod round_trip {
        ...
        fn check(tests: &[(Value, Type)]) {
            for (i, (value, ty)) in tests.iter().enumerate() {
                let b = marshal(value, ty)
                    .unwrap_or_else(|err| panic!("case {i}: marshal error: {err}"));

                let got = unmarshal(&b, ty)
                    .unwrap_or_else(|err| panic!("case {i}: unmarshal error: {err}"));
    ...
    fn round_trip_from_string() {
        ...
        for (i, (value, ty)) in tests.iter().enumerate() {
...

            {
                ...
                let string_got =
                    convert::convert(&original, &Type::string()).unwrap_or_else(|err| {
            ...
            let string_got = convert::convert(&got, &Type::string()).unwrap_or_else(|err| {
                panic!("case {i}: result must be convertible to string: {err}")
            });
    ...
    fn round_trip_truncates_string_prefix_refinement() {
        ...
        for (i, (value, ty, round_trip_value)) in tests.iter().enumerate() {
            let b =
                marshal(value, ty).unwrap_or_else(|err| panic!("case {i}: marshal error: {err}"));
