...
//
// Upstream TestConvert is a single table covering several concepts at once.
...
mod convert_public {
    ...
    fn check(tests: &[(Value, Type, Result<Value, &str>)]) {
        for (i, (value, ty, expect)) in tests.iter().enumerate() {
            ...
            match expect {
                Err(want_err) => match got {
                    Ok(got) => {
                        panic!(
                            "case {i}: conversion succeeded with {got:?}; want error {want_err:?}"
                        )
                    }
                    Err(err) => assert_eq!(
                        err.to_string(),
                ...
                Value::map([
                    (
                        ...
                        Value::object([(
                            ...
                            Value::map([(
                                "nested_foo",
                                Value::object([("x", Value::number(1234.0))]),
                            )]),
                        )]),
                    ),
                    (
    ...
    fn marks() {
        let tests: Vec<(Value, Type, Result<Value, &str>)> = vec![
            ...
            (
                ...
                Type::set(Type::string()),
                Ok(Value::set([Value::string("hello")])
                    .with_marks([ValueMarks::from_marks([1, 2])])),
            ),
            (
