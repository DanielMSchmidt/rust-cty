...
//
// Upstream TestUnify is a single table covering several concepts at once.
...
/// One upstream row: the input types, and the expected unification —
/// `None` for upstream's `(cty.NilType, nil)`, or the unified type plus
/// one "conversion needed" flag per input.
type Case = (Vec<Type>, Option<(Type, Vec<bool>)>);
